	...
	<view class="content">
		...
		<view class="chart">
			...
			<view class="getPhone">
				...
				<input type="text" placeholder-class="place-holder" placeholder="请输入图片验证码" maxlength="4" class="yanzhengma_input" @blur="checkLpicma" v-model="picLyanzhengma">
...
// 			},
			...
			sub(){
				...
				if(this.code.trim()==''||this.pwd.trim()==''||this.pwd1.trim()==''){
					uni.showToast({
						title: '请将信息填写完整!',
						duration:2000,
						icon:'none'
					...
					return;
				}else if(this.pwd.trim()!=this.pwd1.trim()){
					uni.showToast({
						title: '两次密码不一致!',
						duration:2000,
						icon:'none'
